...
- **components/**: Reusable UI elements.
    ...
    - `CompanionDetailOverlay.tsx`: Detailed view for companions allowing leveling up.
    ...
    - `BlackMirrorModal.tsx`: Interface for melting items into Nems currency.
- **views/**: Main game screens.
    - `Home.tsx`: **Container Component**. Handles logic, state, and data fetching for the dashboard. Determines which layout to render.
    - `HomeMobile.tsx`, `HomeTablet.tsx`, `HomeDesktop.tsx`: **Presenter Components**. Pure UI layouts optimized for specific device widths.
    ...
    - `Alchimie.tsx`: Logic for fraction math and potion crafting.
...
- **hooks/**: Custom React hooks.
    - `useDeviceType.ts`: Detects screen width to categorize device as 'mobile', 'tablet', or 'desktop'.
- **services/**: Business logic.
    ...
    - `audioService.ts`: Centralized sound effect management.
...
- **data/**: Static game data.
    ...
    - `companions.ts`: Definitions of companion characters.
    - `statusEffects.ts`: Definitions of Tags (Buffs/Debuffs) for Items, Enemies, and Companions.
...
2. **Responsive Layout Strategy**:
   - The app uses a `useDeviceType` hook to listen for window resize events.
   - `Home.tsx` uses this hook to dynamically switch between `HomeMobile`, `HomeTablet`, and `HomeDesktop` components.
   - This ensures mobile users get a vertical scrolling experience while desktop users get a dashboard view, without CSS media query clutter in a single file.

3. **Combat Systems (Dual Mechanic)**:
...
5. **Entity Tagging System**:
   - **Items**, **Enemies**, and **Companions** share a `tags` array referencing `statusEffects.ts`.
   - `statusService.ts` aggregates these tags to modify runtime values (e.g., `ENEMY_HP_BONUS`, `XP_MULTIPLIER`, `COMBAT_SCORE_BONUS`).
   - Companion leveling automatically upgrades their tag tier (e.g., `scholar_1` -> `scholar_2`).
...
- **Custom CSS**: 
    ...
    - **Animations**: Custom keyframes for `shimmer`, `shake`, `slide-out-blurred-top`, and `roll-in-blurred-top`.
